...
async function getPriorArt() {
  if (!isConfigured()) return [];
  const query = `{node(id:"${PROJECT_ID}"){...on ProjectV2{items(first:20,filter:{direction:ASC,field:UPDATED_AT}){edges{node{id content{...on DraftIssue{id title url}}fieldValues(first:10){edges{node{...on ProjectV2ItemFieldValueSingleSelect{field{...on ProjectV2Field{id}}name}...on ProjectV2ItemFieldValueNumber{field{...on ProjectV2Field{id}}number}}}}}}}}}}}}`;
  try {
    const result = await executeGraphQL(query);
    if (!result?.node) return [];
    const items = result.node.items.edges.map((edge) => {
      const node = edge.node, content = node.content;
      let statusColumn = 'Backlog', phaseNumber = 0;
      node.fieldValues.edges.forEach((fv) => {
        const field = fv.node.field;
        if (field.id === STATUS_FIELD_ID && fv.node.name) statusColumn = fv.node.name;
        if (field.id === PHASE_NUMBER_FIELD_ID && fv.node.number !== undefined) phaseNumber = fv.node.number;
...
module.exports = {
  isConfigured, getPhaseItems, moveCard, setCustomField, commentOnPhaseItem, getPriorArt, GitHubUnreachableError
};
