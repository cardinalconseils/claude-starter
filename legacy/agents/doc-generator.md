---
name: doc-generator
subagent_type: cks:doc-generator
...
**Audience note:** This scope produces `llms.txt` for **coding agents** (Claude Code,
Cursor, Aider) — a machine-first context entry point. DISTINCT from the marketing/GEO
`llms.txt` maintained by `aeo-geo-specialist` for AI search crawlers. Do not confuse the
two; they serve different audiences and must not overwrite each other.

**Collision check (run first):**
If `llms.txt` already exists at project root:
- Read it and display its contents
- Stop with a warning: "llms.txt already exists. Delete or rename it manually, then re-run /cks:docs agent."
- Do NOT overwrite it

**Sources (CKS project — CLAUDE.md present):**
- Project one-liner → CLAUDE.md "What This Project Is" section
- Stack → CLAUDE.md stack section
- Key paths → directory structure + CLAUDE.md project structure block
- Critical conventions → CLAUDE.md constraints section + rule titles from `.claude/rules/`
- Docs index → detect `docs/*.md`, `docs/api/`, `docs/architecture/`
- Commands → `package.json` scripts (dev/test/build) or equivalent manifest

**Sources (non-CKS fallback — no CLAUDE.md):**
- Stack → `package.json` / `pyproject.toml` / `go.mod` / `Cargo.toml` (first found)
- Key paths → top-level directory structure analysis
- Commands → manifest scripts section
- Prepend note: `# Note: CLAUDE.md not found — stack detected from manifest`

**Output:** `./llms.txt` at project root

**Format (terse, ≤40 lines, optimized for agent context window):**
```
...
**Rules:**
- Never emit secret values — read `.env.example` keys only (no values)
- Never overwrite a file without the CKS auto-generated marker (collision check above)
- Every path, command, and version shown must be a real value from the codebase
- Omit a section entirely rather than writing a placeholder

## Staleness Detection

When running with `scope: all`, run all five scopes (api, arch, components, onboarding, agent) and also check for stale docs:
