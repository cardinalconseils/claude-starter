---
description: "Set or list reminders — the chief of staff's proactive wake pushes them to you when due. First reminder registers the recurring wake."
argument-hint: "<when> to <what>  |  list  |  clear"
allowed-tools:
  - Agent
---

# /cks:remind — Reminders for the Proactive Wake

Dispatch the **reminder** agent to save a due-dated reminder under your user memory and,
on your first reminder, get the recurring proactive wake registered — so the chief of
staff pushes the reminder to you (via channel, or surfaced on next session) when it
comes due.
...
3. **One-shot:** if no proactive wake is registered yet, asks cadence once (that is the
   approval — registration is a gated action) and registers it as a Routine, or via
   `CronCreate` as the in-session fallback — later reminders reuse the same wake
4. When due, the chief of staff's proactive wake pushes it out through the channel (or
   surfaces it on the next session for CLI)
