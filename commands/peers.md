...
```
Agent(subagent_type="peer-coordinator", prompt="Peer coordination request. Action: {parsed_action}. Message: {parsed_message}. List peers, show summaries, handle messaging, or check incoming messages as requested. If no peers are available and action is not 'setup', explain that claude-peers-mcp is not configured and offer to run setup. Arguments: $ARGUMENTS")
```

## Quick Reference

```
/cks:peers                 Show active peers and their summaries
/cks:peers setup           Install and configure claude-peers-mcp
/cks:peers send "msg"      Send a message to a peer
