...
**Trigger:** User wants to add one or more existing files to `claude-starter` and make them generic/reusable before committing.

Phrases that trigger this mode:
- "add this to my starter kit"
- "make this virgin / generic / starter-ready"
- "clean this file before pushing"
- "prepare these for the starter repo"

---

### Step 1 — File Collection

Ask the user which files to virginize:

```
/virginize — Prepare files for claude-starter
──────────────────────────────────────────────
Which files do you want to make starter-ready?

Paste file paths or names, one per line.
(e.g., .claude/skills/my-skill.md)
```

Accept 1 to N files. Read each file in full before proceeding.

---

### Step 2 — Detection Pass

For each file, scan for project-specific content and report findings before touching anything:

```
Scanning: .claude/agents/deployer.md
  ⚠ Project name found: "ServiConnect" (3 occurrences)
  ⚠ Hardcoded URL: "https://serviconnect.railway.app"
  ⚠ Specific env var: "TELNYX_API_KEY"
  ⚠ Stack reference: "Telnyx voice API"
  ✓ Structure is clean (role/triggers/tools/constraints/handoff present)

Ready to virginize. Continue?
```
...
### Step 3 — Virginization Rules

Apply these transformations to each file:

| What to find | What to replace with |
|---|---|
| Project name (e.g., "ServiConnect") | `[PROJECT_NAME]` |
| Specific URLs | `[PROJECT_URL]` |
| Hardcoded env var names | `[ENV_VAR_NAME]` |
| Stack-specific tool names in body text | Generic equivalent or `[STACK_TOOL]` |
| Specific DB/table names | `[DATABASE]` / `[TABLE_NAME]` |
| Specific file paths | `[PROJECT_PATH]` |
| Person names / team names | `[TEAM]` |
| Description frontmatter | Rewrite to be fully generic — remove all project context |
| Role section (agents) | Rewrite to generic role, remove project-specific scope |
| Triggers section (agents) | Rewrite to generic trigger conditions |
| Steps section (commands) | Replace project-specific commands with generic equivalents |

**Never remove structural elements** — headings, sections, tables, the shape of the file must survive virginization intact. Only content changes.

**Skill body vs description:**
- `description:` frontmatter → always fully rewritten to generic
- Skill body → preserve structure, replace only project-specific strings

---

### Step 4 — Show Diff Before Saving

For each file, show a before/after summary:

```
── deployer.md ──────────────────────────────
BEFORE (project-specific):
  Role: Deploys ServiConnect to Railway production...
  Triggers: When a PR is merged to main in the ServiConnect repo...

AFTER (starter-ready):
  Role: Deploys [PROJECT_NAME] to the configured deployment platform...
  Triggers: When a PR is merged to the production branch...

3 replacements made. Save this version? (yes / edit / skip)
```

Wait for per-file confirmation before writing.

---

### Step 5 — Output

Write virginized files to `/mnt/user-data/outputs/starter-ready/` preserving their original subfolder structure:

```
starter-ready/
├── skills/
│   └── my-skill.md       ← virginized
└── agents/
    └── deployer.md       ← virginized
```

Then print the git commands to add them to `claude-starter`:

```bash
# Copy files into your local claude-starter repo
cp starter-ready/skills/my-skill.md path/to/claude-starter/.claude/skills/
cp starter-ready/agents/deployer.md path/to/claude-starter/.claude/agents/

# Commit and push
cd path/to/claude-starter
git add .claude/
git commit -m "feat: add my-skill and deployer agent"
git push origin main

# Or if using subtree from a project:
...
### Virginization Quality Check

Before outputting, every virginized file must pass:

- [ ] Zero occurrences of the original project name
- [ ] Zero hardcoded URLs, API keys, or env var values
- [ ] Description frontmatter contains no project-specific context
- [ ] File is immediately usable by `/bootstrap` in any new project
- [ ] Structure (sections, headings, tables) is fully intact

If any check fails, fix before presenting.

See `references/virginize-rules.md` for extended edge cases and examples.
