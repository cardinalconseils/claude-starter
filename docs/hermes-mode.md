...
- **The real work is the conversational brain**, and CKS already has it: the
  `chief-of-staff` skill is the natural-language front-door (Converse / Dispatch /
  Clarify, triage, dispatch), and CKS has file memory, a learning loop, and a persona
  system.
- **The load-bearing decision:** run the chief of staff as the **brain of the top-level
  always-on session**, so it can both *converse* and *dispatch* specialist agents.
...
- **Conversational front-door — the chief of staff.** `skills/chief-of-staff/` (doctrine
  in `SKILL.md`, loop in `SKILL-ORCHESTRATOR.md`, loaded top-level by `/cks:chief` or the
  `CLAUDE.md` channel block). It classifies free text as Converse / Dispatch / Clarify →
  reads the North Star, `.prd/PRD-STATE.md` and memory → triages ACT / DEFER / DROP /
  ESCALATE → opens an issue via `cks:project-manager` → dispatches at most three agents
  from `references/roster.md`. Below ~80% confidence it asks instead of guessing, and its
  output is source-aware (cli / slack / voice / telegram / imessage).
...
CKS today is a **command-dispatch system**: you say a thing, the chief of staff routes it
to a specialist agent, the agent does the work. Hermes is an **always-on assistant**: it
...
**Decision: the chief of staff is the brain of the top-level session, not a sub-agent.**
...
This is the **Orchestrator Exception** in `.claude/rules/commands.md`: a command whose
work must dispatch agents loads a `SKILL-ORCHESTRATOR.md` into the top-level session via
`Skill(skill=...)` instead of dispatching an agent. `/cks:chief` does exactly that with
`skills/chief-of-staff/`, and the `CLAUDE.md` channel block makes the same skill the
brain of a channel session (`skills/chief-of-staff/workflows/channel-mode.md`).

```
Telegram / iMessage message
   → <channel> event in the persistent top-level session
      → CHIEF-OF-STAFF BRAIN (skill loaded at top level, channel mode)
          ├─ small talk / Q&A / advice          → Converse: reply directly via the channel
          ├─ clear work intent (build/ship/fix)  → Dispatch: triage, issue, ≤3 agents, report back
          ├─ ambiguous action                    → Clarify: ask through the channel, set pending
          └─ proactive (scheduled wake fired)    → push a message out via the channel
      → user-scoped memory + conversation-state read on every turn, written by dispatch
```
...
- **Probabilistic plane (judgment is the point):** chief-of-staff routing, conversation,
...
| # | Gap | Today | Target |
|---|-----|-------|--------|
| 1 | **Converse vs. dispatch** | routing only | ✅ Converse / Dispatch / Clarify are the three intent classes in `skills/chief-of-staff/SKILL.md`; Converse is the default for anything that is not an instruction to do work |
...
conversationally through `/cks:chief` (the same front-door that becomes the Telegram
brain):
...
| ✅ **P1 — Conversational brain** | Converse / Dispatch / Clarify intent classes in `skills/chief-of-staff/SKILL.md` | gap #1 |
| ✅ **P1.5 — Channel → brain wiring** | `skills/chief-of-staff/workflows/channel-mode.md` routes inbound channel events through the chief of staff + user-memory, with unattended overrides | this section's runbook |
...
| ✅ **P5 — Proactive wake** | `skills/chief-of-staff/workflows/proactive-wake.md` wakes on a Routine (or `CronCreate` fallback) and pushes blockers / due reminders / stale clarifications out through the channel | gap #4 |
| **P6 — iMessage (optional)** | second host topology B (needs a Mac) | section 1 |

### Channel → brain wiring (runbook)

By default a channel message hits the *raw* session. To route it through the
chief-of-staff brain, add this to the always-on project's `CLAUDE.md` (`/cks:hermes init`
does it), then launch with `--channels`:
...
### Proactive wake (runbook)

By default the agent only replies. To make it **initiate** messages, register a recurring
wake that re-enters the session with a proactive prompt. Registration is a gated action:
a role proposes it, the chief of staff registers it as a Routine after approval
(`CronCreate` remains the in-session fallback):

```text
Proactive wake for user <slug>. Follow skills/chief-of-staff/workflows/proactive-wake.md:
...
Key constraints (enforced by `skills/chief-of-staff/workflows/proactive-wake.md`):
...
- Reminders live at `~/.cks/user/<slug>/reminders.md` (`$CKS_HQ/users/<slug>/` when an
  HQ is set); cadence (hourly default) is the user's preference, confirmed before the
  chief of staff registers the wake.

### Cloud sessions and Routines

Hermes over Telegram is now the **secondary** lane. The primary front door is a Claude
Code web/mobile **cloud session**: the environment setup script installs CKS, the HQ repo
(`cardinalconseils/hq`) is the default session source, and permission prompts reach the
app — so the human backstop stays in place without `--dangerously-skip-permissions`. The
first turn of an HQ session loads `Skill(skill="cks:chief-of-staff")`, the same brain a
channel session runs.

**Routines** — Claude Code Remote triggers, with connectors and push notifications —
replace `CronCreate` for anything that must outlive a session. `CronCreate` is
session-bound and dies with the process; a Routine fires a fresh session on schedule.
The owner already runs daily learnings curation, a daily CCCS threat scan and a weekly
ecosystem watch this way. Registering, changing or pausing a Routine is a gated action:
any role proposes, only the chief of staff registers, starting at autonomy Level 1 or 2
(`.claude/rules/loops.md`). Sprint 2 ships `skills/routines/` with the profile format.

Telegram via channels stays as the secondary lane, unchanged at runtime, with one change
of address: the VPS session's working directory is the **HQ clone**, so North Star,
memory and routine state are git-pushed rather than trapped on the host. Both lanes use
the same gated-action list in `skills/chief-of-staff/SKILL.md`.
...
- **Permission-prompt stalls (confirmed)** — empirically, with fakechat: a message that
  ...
  plugin that relays prompts. This is why channel mode forbids `AskUserQuestion`.
...
- CKS internals cited above: `skills/chief-of-staff/SKILL.md`,
