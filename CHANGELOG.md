...
- Chief of Staff pattern (coordinator / executors / verifier) in the chief-of-staff skill — `workflows/verify.md` (red gate before a build or fix dispatch; after every return the diff is read, every claimed command is re-run by `cks:tester` at Level 1 with errored-check and positive-control rules, artifacts are read back) and `workflows/sessions.md` (executing work as separate Claude Code Remote sessions with an issue card as the report, `permission_mode: "auto"`, a 15–30 minute check-in armed with `send_later`, and archive only after PROVE PASS); wired into `SKILL-ORCHESTRATOR.md` step 5 and routine mode
- `docs/v6-workforce.md` maps the 18 roles onto the coordinator / executor / verifier / reporter / board seats
