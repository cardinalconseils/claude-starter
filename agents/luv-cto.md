...
Agent(subagent_type="cks:luv-tech-lead", prompt="[Sprint/roadmap request: features to plan, dependencies, deadline, priority ranking.]")
Agent(subagent_type="cks:luv-api-designer", prompt="[API design request: endpoints needed, data models, auth requirements, consumers (frontend/mobile/partner).]")

# Development
Agent(subagent_type="cks:luv-frontend-dev", prompt="[Frontend task: component or page, design spec, API endpoints to integrate, performance targets.]")
Agent(subagent_type="cks:luv-backend-dev", prompt="[Backend task: endpoint to build, data model, auth requirements, test coverage expected.]")
Agent(subagent_type="cks:luv-full-stack-dev", prompt="[Full-stack task: feature description, affected layers, third-party integration details.]")
Agent(subagent_type="cks:luv-mobile-app-dev", prompt="[Mobile task: platform (iOS/Android/both), feature, backend API contract, UX spec.]")
Agent(subagent_type="cks:luv-landing-page-dev", prompt="[Landing page: goal, copy brief, tracking requirements, deadline.]")

# Infrastructure
Agent(subagent_type="cks:luv-devops", prompt="[Infra request: environment, service, scaling need or security hardening task.]")
Agent(subagent_type="cks:luv-cicd", prompt="[Pipeline task: workflow to create or fix, trigger condition, deployment target.]")
Agent(subagent_type="cks:luv-database-auth-engineer", prompt="[DB/auth task: schema change, RLS policy, migration requirement, compliance concern.]")

# AI Tooling
Agent(subagent_type="cks:luv-ai-tooling-engineer", prompt="[AI task: model to integrate, prompt engineering challenge, evaluation requirement, cost constraint.]")

# Quality
Agent(subagent_type="cks:luv-qa-engineer", prompt="[QA task: what to review, acceptance criteria, agent output or workflow to validate.]")
Agent(subagent_type="cks:luv-uat-engineer", prompt="[UAT task: user journey to test, platform, tracking to validate, go/no-go criteria.]")
Agent(subagent_type="cks:luv-debugger", prompt="[Debug task: error description, stack trace or logs, steps to reproduce, affected system.]")
Agent(subagent_type="cks:luv-agent-browser", prompt="[Browser task: URL to navigate, data to extract or action to perform, expected output format.]")
```
